...
use Rvx\Twig\Node\Expression\MacroReferenceExpression;
...
class ExpressionParser
{
    ...
    private bool $deprecationCheck = \true;
    public function __construct(private Parser $parser, private Environment $env)
    {
    ...
    public function parseExpression($precedence = 0)
    {
        ...
        while ($this->isBinary($token) && $this->binaryOperators[$token->getValue()]['precedence'] >= $precedence) {
            ...
            if ('is not' === $token->getValue()) {
                ...
                $previous = $this->setDeprecationCheck(\true);
                try {
    ...
    private function parseSubscriptExpressionDot(Node $node) : AbstractExpression
    {
        ...
        if ($node instanceof NameExpression && (null !== $this->parser->getImportedSymbol('template', $node->getAttribute('name')) || '_self' === $node->getAttribute('name') && $attribute instanceof ConstantExpression)) {
