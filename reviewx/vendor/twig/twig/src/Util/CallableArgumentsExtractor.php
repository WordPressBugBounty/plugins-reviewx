...
use Rvx\Twig\Node\Expression\VariadicExpression;
...
final class CallableArgumentsExtractor
{
    ...
    public function __construct(private Node $node, private TwigCallableInterface $twigCallable)
    {
    ...
    public function extractArguments(Node $arguments) : array
    {
        ...
        if ($extractedArguments) {
            ...
            throw new SyntaxError(\sprintf('Unknown argument%s "%s" for %s "%s(%s)".', \count($extractedArguments) > 1 ? 's' : '', \implode('", "', \array_keys($extractedArguments)), $this->twigCallable->getType(), $this->twigCallable->getName(), \implode(', ', \array_map([$this, 'toSnakeCase'], $callableParameterNames))), $unknownArgument ? $unknownArgument->getTemplateLine() : $this->node->getTemplateLine(), $unknownArgument ? $unknownArgument->getSourceContext() : $this->node->getSourceContext());
        }
        return $arguments;
    }
    ...
    {
        return \strtolower(\preg_replace(['/([A-Z]+)([A-Z][a-z])/', '/([a-z0-9])([A-Z])/'], 'Rvx\\1_\\2', $name));
    }
