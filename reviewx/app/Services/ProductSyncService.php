...
class ProductSyncService extends \Rvx\Services\Service
{
    ...
    public function syncPost($file)
    {
        ...
        DB::table('posts')->select(['ID', 'post_type', 'post_title', 'post_name', 'post_content', 'post_status', 'post_modified'])->orderBy('ID')->whereIn('post_type', $this->allowedPostTypes)->chunk(100, function ($products) use(&$attachmentRelation, &$product_url_base, &$file, &$productCount) {
