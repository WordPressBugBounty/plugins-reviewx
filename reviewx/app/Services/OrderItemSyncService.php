...
class OrderItemSyncService extends \Rvx\Services\Service
{
    ...
    public function formatOrderData($order) : array
    {
        ...
        return ['rid' => 'rid://Order/' . (int) $order->id, "wp_id" => (int) $order->id, "customer_id" => $order->customer_id ? Client::getUid() . '-' . $order->customer_id : null, "subtotal" => (float) ($order->total_amount ?? 0.0), "tax" => (float) ($order->tax_amount ?? 0.0), "total" => (float) ($order->total_amount ?? 0.0), "status" => isset($order->status) ? Helper::orderStatus(Helper::rvxGetOrderStatus($order->status)) : null, "review_request_email_sent_at" => null, "review_reminder_email_sent_at" => null, "photo_review_email_sent_at" => null, "paid_at" => $paid_at, 'created_at' => !empty($order->date_created_gmt) ? \wp_date('Y-m-d H:i:s', \strtotime($order->date_created_gmt)) : null, 'updated_at' => !empty($order->date_updated_gmt) ? \wp_date('Y-m-d H:i:s', \strtotime($order->date_updated_gmt)) : null];
    }
    public function formatOrderItem($orderItem) : array
    {
        return ['rid' => 'rid://LineItem/' . (int) $orderItem->order_item_id, 'wp_id' => (int) $orderItem->order_item_id, 'order_id' => (int) $orderItem->order_id, 'product_wp_unique_id' => Client::getUid() . '-' . (int) $orderItem->product_id, 'name' => $orderItem->order_item_name ?? null, 'quantity' => (int) ($orderItem->quantity ?? 0), 'price' => (float) ($orderItem->price ?? 0.0), 'review_id' => null, 'site_id' => Client::getSiteId(), 'fulfillment_status' => $this->orderFullfillmentStatusRelation[(int) $orderItem->order_id] ?? null, 'fulfilled_at' => !empty($this->orderFullfillmentAtRelation[(int) $orderItem->order_id]) ? \wp_date('Y-m-d H:i:s', \strtotime($this->orderFullfillmentAtRelation[(int) $orderItem->order_id])) : null, 'reviewed_at' => null];
    }
